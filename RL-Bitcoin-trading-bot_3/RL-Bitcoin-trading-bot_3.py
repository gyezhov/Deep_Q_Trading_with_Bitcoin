...
class CustomEnv:
    ...
    def step(self, action):
        ...
        current_price = random.uniform(
            ...
            self.df.loc[self.current_step, 'Close'])
        Date = self.df.loc[self.current_step, 'Timestamp'] # for visualization
        ...
        if action == 0: # Hold
            ...
            self.trades.append({'Timestamp' : Date, 'High' : High, 'Low' : Low, 'total': self.crypto_bought, 'type': "buy"})
            ...
            self.trades.append({'Timestamp' : Date, 'High' : High, 'Low' : Low, 'total': self.crypto_sold, 'type': "sell"})
            self.episode_orders += 1
    ...
    def render(self, visualize = False):
        ...
        if visualize:
            Date = self.df.loc[self.current_step, 'Timestamp']
...
df = df.loc[df.index < pd.datetime(2017, 7, 1)]
df.drop(['Timestamp', 'Volume_(Currency)', 'Weighted_Price'], axis=1, inplace=True)
df.rename(columns={'Volume_(BTC)': 'Volume'}, inplace=True)
df = df.groupby(pd.Grouper(freq="H")).aggregate({
            'Open': 'first',
            'High': 'max',
            'Low': 'min',
            'Close': 'last',
