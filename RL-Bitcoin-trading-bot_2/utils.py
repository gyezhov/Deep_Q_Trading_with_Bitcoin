...
def Write_to_file(Timestamp, net_worth, filename='{}.txt'.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))):
    for i in net_worth: 
        Timestamp += " {}".format(i)
    #print(Timestamp)
...
class TradingGraph:
    ...
    # Timestamp, Open, High, Low, Close, Volume, net_worth, trades
    ...
    def render(self, Timestamp, Open, High, Low, Close, Volume, net_worth, trades):
        # append volume and net_worth to deque list
        self.Volume.append(Volume)
        self.net_worth.append(net_worth)

        # before appending to deque list, need to convert Timestamp to special format
        Timestamp = mpl_dates.date2num([pd.to_datetime(Timestamp)])[0]
        self.render_data.append([Timestamp, Open, High, Low, Close])
        ...
        # beautify the x-labels (Our Timestamp format)
        self.ax1.xaxis.set_major_formatter(self.date_format)
        self.fig.autofmt_xdate()

        # sort sell and buy orders, put arrows in appropiate order positions
        for trade in trades:
            trade_date = mpl_dates.date2num([pd.to_datetime(trade['Timestamp'])])[0]
