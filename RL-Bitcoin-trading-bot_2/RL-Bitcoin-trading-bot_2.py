...
class CustomEnv:
    ...
    def step(self, action):
        ...
        self.orders_history.append([self.balance, self.net_worth, self.crypto_bought, self.crypto_sold, self.crypto_held])
        #Write_to_file(Timestamp, self.orders_history[-1])
    ...
    def render(self, visualize = False):
        ...
        if visualize:
            ...
            self.visualization.render(Timestamp, Open, High, Low, Close, Volume, self.net_worth, self.trades)
...
df = df.groupby(pd.Grouper(freq='1Min')).aggregate({
            'Open': 'first',
            'High': 'max',
            'Low': 'min',
            'Close': 'last',
